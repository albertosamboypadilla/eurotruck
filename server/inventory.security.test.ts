...
describe("inventory procedures", () => {
  ...
  it("permite registrar una salida a admin1 y rechaza admin2", async () => {
    await expect(appRouter.createCaller(context(admin1)).inventory.recordSale({ sku: "SKU-1", quantity: 1, source: "scan", confirmationKey: "1989" })).resolves.toMatchObject({ sku: "SKU-1", totalQuantity: 4 });
    expect(recordInventorySale).toHaveBeenCalledWith({ sku: "SKU-1", quantity: 1, source: "scan", confirmationKey: "1989", movedBy: "admin1" });
    await expect(appRouter.createCaller(context(admin2)).inventory.recordSale({ sku: "SKU-1", quantity: 1, source: "scan", confirmationKey: "1989" })).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
