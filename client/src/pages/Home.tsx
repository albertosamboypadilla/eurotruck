...
const gtinMapFileUrl = "/manus-storage/diesel-gtin-map_07ac3c19.json";
  ...
  async function submitRegistration(event: FormEvent<HTMLFormElement>) {
    ...
    if (validationError) {
      const validationErrorEnglish: Record<string, string> = {
        ...
        "Agrega al menos una pieza al carrito antes de generar la orden.": "Add at least one part to the cart before generating the order.",
      };
    ...
    try {
      ...
      setOrderError(t("No pudimos registrar la orden. Revisa los datos e inténtalo nuevamente.", "We could not register the order. Check the details and try again."));
    }
  }
    ...
    <div className="site-shell">
      <header className="site-header">
        ...
        <div className="header-top container-wide">
          ...
          <form className="portal-header-search" onSubmit={submitCatalogSearch}><span>{t("Buscar piezas", "Search parts")}</span><div><input aria-label={t("Buscar piezas", "Search parts")} value={catalogSearch} onChange={(event) => setCatalogSearch(event.target.value)} placeholder={t("N.º de referencia, GTIN o descripción (DT, SA, OE, etc.)", "Part number, GTIN or description (DT, SA, OE, etc.)")} /><button type="submit" aria-label={t("Buscar en el catálogo", "Search catalog")}><Search size={19} /></button></div></form>
          <div className="header-tools">
            ...
            <div className="utility-row">
              ...
              <a className="company-access-button" href="/orders" aria-label={t("Abrir acceso de empresa y bandeja de órdenes", "Open company access and order inbox")}><ShieldCheck size={14} />{t("Acceso empresa", "Company access")}</a><button className="cart-button" onClick={() => setCartOpen(true)}><ShoppingCart size={14} />{t("Carrito", "Cart")} <b>{cartCount}</b></button>
            </div>
      ...
      <main>
        <section id="inicio" className="hero-section">
          ...
          <div id="hero-fleet" className="fleet-stage container-wide" onMouseEnter={() => setIsFleetPaused(true)} onMouseLeave={() => setIsFleetPaused(false)} onFocus={() => setIsFleetPaused(true)} onBlur={() => setIsFleetPaused(false)}>
            ...
            <div className="hero-ctas">
              ...
              <button className="cta cta-green" onClick={() => scrollToId("registro")}><ClipboardList size={15} />{t("Registrar Pedido de Pieza", "Register Parts Request")}</button>
        ...
        <section className="registration-section" id="registro">
          <div className="container-wide registration-layout">
            <div className="registration-copy"><SectionLabel>{t("REGISTRO RÁPIDO DE PEDIDOS", "QUICK ORDER REGISTRATION")}</SectionLabel><h2>{t("Registra tu Empresa", "Register Your Company")}<br /><span>{t("para Pedidos de Piezas", "for Parts Orders")}</span></h2><p>{t("Solo necesitamos estos datos para procesar tus despachos y cotizaciones de inmediato.", "We only need these details to process your shipments and quotes right away.")}</p><div className="registration-promises"><span><Check size={14} />{t("Atención prioritaria y despacho el mismo día", "Priority service and same-day dispatch")}</span><span><Check size={14} />{t("Opción de instalación en taller o a domicilio", "Workshop or on-site installation available")}</span></div><div className="registration-photo"><div /><small>{t("RESPUESTA DIRECTA PARA TU FLOTA", "DIRECT RESPONSE FOR YOUR FLEET")}</small></div></div>
            <form className="registration-form" onSubmit={submitRegistration}>
              ...
              <button className="submit-button" type="submit" disabled={createOrderMutation.isPending}>{submitted ? <><Check size={16} />{t(`Orden ${submittedOrder} descargada — te contactaremos`, `Order ${submittedOrder} downloaded — we will contact you`)}</> : createOrderMutation.isPending ? <>{t("Generando orden…", "Generating order…")}</> : <>{t("Enviar solicitud y descargar PDF", "Send request and download PDF")} <Send size={15} /></>}</button>
            ...
            <div className="product-grid">{visibleProducts.map((product) => {
              ...
              return <article className="product-card" key={product.id}>
                <button className="product-open" onClick={() => setSelectedProduct(product)} aria-label={t(`Ver ${product.name}, referencia ${product.sku}`, `View ${product.name}, part number ${product.sku}`)}>
                  ...
                  <div className="product-info"><div className="product-kicker"><span>{product.category}</span><span>{product.brand}</span><b>{product.sku}</b>{product.gtins?.length ? <small className="product-gtin-label"><Barcode size={11} />GTIN {product.gtins.join(" · ")}</small> : <small className="product-gtin-label product-gtin-label--missing"><Barcode size={11} />{t("GTIN no registrado", "GTIN not registered")}</small>}</div>{productBadges.length > 0 && <div className="product-badges">{productBadges.slice(0, 3).map((badge) => <span key={badge}>{badge}</span>)}</div>}<h3>{product.name}</h3><div className="product-source-facts"><span><small>{t("Reemplaza", "Replaces")}</small>{product.replaces || "—"}</span><span><small>{t("Adecuado para", "Suitable for")}</small>{product.application}</span><span><small>{t("Empaque", "Pack")}</small>{product.packagingAmount} {product.salesUnit}</span><span><small>{t("Precio neto", "Net price")}</small>{t("Bajo demanda", "On request")}</span></div><div className={`product-location${inventoryLocation ? "" : " product-location--pending"}`}><MapPin size={12} /><span><small>{t("Ubicación", "Location")}</small>{inventoryLocation ? `${inventoryLocation.lastTramo || "—"} / ${inventoryLocation.lastGondola || "—"}` : t("Pendiente de conteo", "Pending count")}</span></div><div className="product-footer"><span className="stock"><span className="status-dot" />{product.application}</span><span className="product-more">{t("Más información", "More information")} <ArrowUpRight size={13} /></span></div></div>
                </button>
