...
type PublicInventoryLocation = { productId: string; totalQuantity: number; lastTramo: string | null; lastGondola: string | null };

const catalogFileUrl = "/manus-storage/catalog-with-junta-torica-gtin-20260904_40163118.json";
...
export default function Home() {
  ...
  const isInventoryOperator = user?.role === "admin" && (user.name || "").trim().toLowerCase() === "admin1";
  ...
  const [bulkSaleQuantities, setBulkSaleQuantities] = useState<Record<string, string>>({});
  ...
  function submitHomeSale(event: FormEvent<HTMLFormElement>) {
    ...
    if (!Number.isInteger(quantity) || quantity < 1 || !isInventorySaleConfirmationKey(saleKey)) {
      setSaleError(!isInventorySaleConfirmationKey(saleKey) ? "Clave incorrecta. La salida no fue registrada." : "Indica una cantidad válida.");
      return;
    }
    recordSaleMutation.mutate({ productId: saleProduct.id, sku: saleProduct.sku, quantity, confirmationKey: saleKey, source: "manual" });
  }

  function toggleSaleSelection(productId: string) {
    setSelectedSaleIds(current => current.includes(productId) ? current.filter(id => id !== productId) : [...current, productId]);
  }

  function openBulkSaleDialog() {
    if (selectedSaleIds.length === 0) return;
    setBulkSaleQuantities(Object.fromEntries(selectedSaleIds.map(id => [id, "1"])));
    setBulkSaleKey("");
    setBulkSaleError("");
    setBulkSaleOpen(true);
  }

  async function submitBulkSale(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!isInventorySaleConfirmationKey(bulkSaleKey)) {
      setBulkSaleError("Clave incorrecta. No se descontó ningún artículo.");
      return;
    }
    const selectedProducts = catalog.filter(product => selectedSaleIds.includes(product.id));
    const quantities = selectedProducts.map(product => ({ product, quantity: Number.parseInt(bulkSaleQuantities[product.id] || "0", 10) }));
    if (quantities.some(({ quantity }) => !Number.isInteger(quantity) || quantity < 1)) {
      setBulkSaleError("Indica una cantidad válida para cada artículo seleccionado.");
      return;
    }
    try {
      for (const { product, quantity } of quantities) {
        await recordSaleMutation.mutateAsync({ productId: product.id, sku: product.sku, quantity, confirmationKey: bulkSaleKey, source: "manual" });
      }
      setSelectedSaleIds([]);
      setBulkSaleOpen(false);
      setBulkSaleKey("");
      setBulkSaleQuantities({});
      setBulkSaleError("");
    } catch (error) {
      setBulkSaleError(error instanceof Error ? error.message : "No se pudo completar el descuento por lote.");
    }
  }
  ...
  return (
    <div className="site-shell">
      ...
      <main>
        ...
        <section className="catalog-section" id="catalogo">
          ...
          <div className="container-wide catalog-content">
            ...
            <div className="catalog-meta"><span><PackageCheck size={14} />{catalogLoading ? t("Cargando catálogo...", "Loading catalog...") : `${formatCount(applicationResultCount)} ${t("resultados de la aplicación", "application results")}`}</span><span><Barcode size={14} />{catalogLoading ? t("Cargando GTIN…", "Loading GTINs…") : `${Object.keys(gtinMap).length.toLocaleString("es-DO")} ${t("referencias con GTIN verificado", "references with verified GTIN")}`}</span></div>
            ...
            <div className="product-grid">{visibleProducts.map((product) => {
              const inventoryLocation = inventoryLocations.get(product.id);
              ...
              return <article className={selectedSaleIds.includes(product.id) ? "product-card is-sale-selected" : "product-card"} key={product.id}>
                {isInventoryOperator && <label className="product-select-control" onClick={event => event.stopPropagation()}><input type="checkbox" checked={selectedSaleIds.includes(product.id)} onChange={() => toggleSaleSelection(product.id)} aria-label={`Seleccionar ${product.name} para descontar`} /><span>Seleccionar</span></label>}
            ...
            {filteredProducts.length > 0 && <><div className="catalog-bulk-actions">{isInventoryOperator && <><label><input type="checkbox" checked={visibleProducts.length > 0 && visibleProducts.every(product => selectedSaleIds.includes(product.id))} onChange={event => setSelectedSaleIds(current => event.target.checked ? Array.from(new Set([...current, ...visibleProducts.map(product => product.id)])) : current.filter(id => !visibleProducts.some(product => product.id === id)))} />{t("Seleccionar todo", "Select all")}</label><button className="catalog-bulk-sale-button" disabled={selectedSaleIds.length === 0} onClick={openBulkSaleDialog}><TrendingDown size={13} />{t("Descontar cantidad", "Deduct quantity")} {selectedSaleIds.length > 0 ? `(${selectedSaleIds.length})` : ""}</button></>}<button>{t("Comparar producto", "Compare product")}</button><button>{t("Añadir a bloc de notas", "Add to notes")}</button><button onClick={() => scrollToId("registro")}><ShoppingCart size={13} />{t("Añadir al carrito", "Add to cart")}</button></div><div className="catalog-pagination"><button disabled={catalogPage === 1} onClick={() => setCatalogPage((page) => Math.max(1, page - 1))}><ArrowLeft size={14} />{t("Anterior", "Previous")}</button><span>{t("Página", "Page")} <b>{catalogPage}</b> {t("de", "of")} <b>{pageCount}</b></span><button disabled={catalogPage === pageCount} onClick={() => setCatalogPage((page) => Math.min(pageCount, page + 1))}>{t("Siguiente", "Next")}<ArrowRight size={14} /></button></div></>}
            </>}
          </div>
        </section>
      ...
      {selectedProduct && <div className="product-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedProduct(null); }}><div className="product-modal product-modal--enhanced" role="dialog" aria-modal="true" aria-labelledby="product-modal-title"><button className="product-modal-close" onClick={() => setSelectedProduct(null)} aria-label={t("Cerrar detalle", "Close details")}><X size={17} /></button><div className="product-modal-media">{selectedProduct.imageFull || selectedProduct.image ? <img src={selectedProduct.imageFull || selectedProduct.image} alt={`${selectedProduct.name} — ${selectedProduct.sku}`} /> : <div className="product-no-image"><PackageCheck size={48} /><span>{t("Imagen no disponible en la fuente", "Image unavailable from source")}</span></div>}<span className="product-modal-zoom-note">{t("Pasa el cursor para ampliar", "Hover to zoom")}</span></div><div className="product-modal-copy"><div className="product-modal-heading"><SectionLabel>{t("FICHA DEL ARTÍCULO", "PART PROFILE")}</SectionLabel>{isAuthorizedViewer && <span className="product-modal-sku">SKU {selectedProduct.sku}</span>}</div><h2 id="product-modal-title">{selectedProduct.name}</h2>{isAuthorizedViewer && <><p className="product-modal-description">{selectedProduct.description}</p>{(() => { const inventory = inventoryLocations.get(selectedProduct.id); return <div className={`product-modal-availability${inventory?.totalQuantity ? inventory.totalQuantity <= 3 ? " is-low" : " is-available" : " is-pending"}`}><div><PackageCheck size={22} /><span><small>{t("Existencia en almacén", "Warehouse stock")}</small><strong>{inventory?.totalQuantity ? `${inventory.totalQuantity} ${t("unidades", "units")}` : t("Sin existencia registrada", "No stock recorded")}</strong></span></div><div><MapPin size={19} /><span><small>{t("Ubicación", "Location")}</small><strong>{inventory ? `${inventory.lastTramo || "—"} / ${inventory.lastGondola || "—"}` : t("Pendiente", "Pending")}</strong></span></div><div><span><small>{t("Precio", "Price")}</small><strong>{t("Solicitar cotización", "Request a quote")}</strong></span></div></div>; })()}<div className="product-modal-facts"><span><b>{t("Marca", "Brand")}</b>{selectedProduct.brand}</span><span><b>{t("Aplicación", "Application")}</b>{selectedProduct.usage.slice(0, 3).join(", ") || selectedProduct.manufacturer || "Multibrand"}</span><span><b>{t("Categoría", "Category")}</b>{selectedProduct.category}</span><span><b>{t("Empaque", "Pack")}</b>{selectedProduct.packagingAmount} {selectedProduct.salesUnit}</span><span><b>GTIN</b>{selectedProduct.gtins?.join(" · ") || t("No registrado", "Not registered")}</span><span><b>{t("Código interno Zebra", "Internal Zebra code")}</b>{selectedProduct.internalCode || "No asignado"}</span><span><b>{t("Código de barras", "Barcode")}</b>{selectedProduct.barcode || selectedProduct.gtins?.[0] || t("No registrado", "Not registered")}</span><span><b>{t("Referencia interna", "Internal reference")}</b>{selectedProduct.id}</span></div>{selectedProduct.replaces && <p className="product-modal-replaces"><b>{t("Reemplaza:", "Replaces:")}</b> {selectedProduct.replaces}</p>}</>}<div className="product-modal-actions">{isAuthorizedViewer && <a className="modal-source-link" href={selectedProduct.url} target="_blank" rel="noreferrer">{t("Ver fuente técnica", "View technical source")} <ArrowUpRight size={14} /></a>}{isInventoryOperator && <button className="inventory-home-sale-button catalog-bulk-sale-button" type="button" onClick={() => { setSaleProduct(selectedProduct); setSaleError(""); setSaleKey(""); setSaleQuantity("1"); }}><TrendingDown size={14} />Descontar cantidad</button>}<button className="modal-add-button" onClick={() => { addToCart(selectedProduct); setSelectedProduct(null); setCartOpen(true); }}><Plus size={14} />{t("Agregar a cotización", "Add to quote")}</button></div></div></div></div>}

      {saleProduct && <div className="inventory-sale-dialog" role="dialog" aria-modal="true" aria-labelledby="home-sale-dialog-title"><form className="inventory-sale-dialog-card" onSubmit={submitHomeSale}><div className="inventory-sale-dialog-icon"><TrendingDown size={22} /></div><span className="orders-eyebrow">SALIDA DE ALMACÉN</span><h2 id="home-sale-dialog-title">Descontar cantidad</h2><p>Se descontará únicamente del inventario la cantidad indicada para <strong>{saleProduct.sku}</strong>. El artículo base no se elimina.</p><label>Cantidad<input type="number" min="1" max="9999" value={saleQuantity} onChange={event => setSaleQuantity(event.target.value)} autoFocus /></label><label>Contraseña de confirmación<input type="password" inputMode="numeric" maxLength={4} value={saleKey} onChange={event => setSaleKey(event.target.value.replace(/\D/g, "").slice(0, 4))} autoComplete="off" /></label>{saleError && <div className="inventory-alert inventory-alert--error" role="alert"><AlertTriangle size={16} />{saleError}</div>}<div className="inventory-sale-dialog-actions"><button type="button" className="orders-button orders-button--ghost" onClick={() => { setSaleProduct(null); setSaleError(""); }}>Cancelar</button><button type="submit" className="orders-button inventory-row-sale-confirm catalog-bulk-sale-button" disabled={recordSaleMutation.isPending}>{recordSaleMutation.isPending ? "Procesando…" : "Confirmar descuento"}</button></div></form></div>}

      {bulkSaleOpen && <div className="inventory-sale-dialog" role="dialog" aria-modal="true" aria-labelledby="bulk-sale-dialog-title"><form className="inventory-sale-dialog-card inventory-bulk-sale-card" onSubmit={submitBulkSale}><div className="inventory-sale-dialog-icon"><TrendingDown size={22} /></div><span className="orders-eyebrow">SALIDA MÚLTIPLE DE ALMACÉN</span><h2 id="bulk-sale-dialog-title">Descontar cantidad seleccionada</h2><p>La operación solo descuenta cantidades y no elimina los artículos del catálogo. Revisa cada cantidad antes de confirmar.</p><div className="bulk-sale-lines">{catalog.filter(product => selectedSaleIds.includes(product.id)).map(product => <label className="bulk-sale-line" key={product.id}><span><strong>{product.name}</strong><small>DT Spare Parts / SKU {product.sku}</small></span><input type="number" min="1" max="9999" value={bulkSaleQuantities[product.id] || "1"} onChange={event => setBulkSaleQuantities(current => ({ ...current, [product.id]: event.target.value }))} aria-label={`Cantidad a descontar de ${product.name}`} /></label>)}</div><label>Contraseña de confirmación<input type="password" inputMode="numeric" maxLength={4} value={bulkSaleKey} onChange={event => setBulkSaleKey(event.target.value.replace(/\D/g, "").slice(0, 4))} autoComplete="off" autoFocus /></label>{bulkSaleError && <div className="inventory-alert inventory-alert--error" role="alert"><AlertTriangle size={16} />{bulkSaleError}</div>}<div className="inventory-sale-dialog-actions"><button type="button" className="orders-button orders-button--ghost" onClick={() => { setBulkSaleOpen(false); setBulkSaleError(""); }}>Cancelar</button><button type="submit" className="orders-button inventory-row-sale-confirm catalog-bulk-sale-button" disabled={recordSaleMutation.isPending}>{recordSaleMutation.isPending ? "Procesando…" : `Confirmar ${selectedSaleIds.length} artículo${selectedSaleIds.length === 1 ? "" : "s"}`}</button></div></form></div>}
