...
import { AlertTriangle, Archive, CalendarDays, Calculator, ClipboardList, DollarSign, FileDown, Hand, House, LoaderCircle, LogIn, LogOut, PackageSearch, RefreshCw, Save, Trash2, TrendingDown } from "lucide-react";
...
export default function Orders() {
  ...
  const [dailySalesDate, setDailySalesDate] = useState(() => new Date().toISOString().slice(0, 10));
  ...
  const quotePricesQuery = trpc.inventory.quotePrices.useQuery(undefined, { enabled: user?.role === "admin", staleTime: 5_000 });
  const availability = useMemo(() => new Map((availabilityQuery.data || []).map(item => [item.productId, item] as const)), [availabilityQuery.data]);
  const quotePrices = useMemo(() => new Map((quotePricesQuery.data || []).map(item => [item.productId, item.salePrice] as const)), [quotePricesQuery.data]);
  ...
  const dailySalesQuery = trpc.inventory.dailySales.useQuery({ date: dailySalesDate }, { enabled: showDailySales && user?.role === "admin", retry: false });
  ...
  const openQuote = (id: number, items: DraftLine[]) => {
    ...
    setExpanded(id);
    setDrafts(current => current[id] ? current : {
      ...current,
      [id]: items.map(item => {
        const inventoryPrice = quotePrices.get(item.productId);
        const syncedPrice = Number(inventoryPrice || 0) > 0 ? inventoryPrice : item.unitPrice;
        return { ...item, unitPrice: String(syncedPrice ?? "0.00") };
      }),
    });
  };
  ...
  return <main className="orders-page quotes-page">
    <header className="orders-header"><div><span className="orders-eyebrow">EUROTRUCK / COTIZACIONES</span><h1>{isDeletedView ? "Historial" : "Cotizaciones"}</h1><p>{isDeletedView ? "Cotizaciones archivadas para consulta administrativa." : "Revisa existencias, ajusta cantidades y precios, y genera el PDF final."}</p></div><div className="orders-header-actions"><a className="orders-button orders-button--ghost" href="/"><House size={15} />Página principal</a>{user.name?.toLowerCase() === "admin1" && <a className="orders-button orders-button--inventory" href="/inventory"><PackageSearch size={15} />Inventario</a>}<button className={`orders-button ${showDailySales ? "orders-button--inventory" : "orders-button--ghost"}`} onClick={() => setShowDailySales(current => !current)}><TrendingDown size={15} />Venta del día</button><button className="orders-button orders-button--ghost" onClick={() => { void ordersQuery.refetch(); void availabilityQuery.refetch(); void quotePricesQuery.refetch(); if (isDeletedView) void deletedOrdersQuery.refetch(); if (showDailySales) void dailySalesQuery.refetch(); }}><RefreshCw size={15} />Actualizar</button><button className="orders-button orders-button--ghost" onClick={() => localLogout.mutate()}><LogOut size={15} />Salir</button></div></header>
    ...
    {showDailySales ? <section className="daily-sales-panel" aria-labelledby="daily-sales-title"><div className="daily-sales-header"><div><span className="orders-eyebrow">EUROTRUCK / SALIDAS</span><h2 id="daily-sales-title">Venta del día</h2><p>Salidas registradas desde Inventario con precio final vigente y numeración consecutiva por fecha.</p></div><div className="daily-sales-date"><CalendarDays size={17} /><label>Fecha<input type="date" value={dailySalesDate} onChange={event => setDailySalesDate(event.target.value)} /></label></div></div>{dailySalesQuery.isLoading ? <div className="orders-state"><LoaderCircle className="orders-spin" />Cargando ventas…</div> : dailySalesQuery.isError ? <div className="orders-state"><h2>No se pudo cargar Venta del día</h2><p>Verifica tu sesión autorizada e inténtalo nuevamente.</p></div> : <><div className="daily-sales-summary"><span><small>Salidas</small><strong>{dailySalesQuery.data?.length ?? 0}</strong></span><span><small>Unidades</small><strong>{(dailySalesQuery.data || []).reduce((sum, item) => sum + item.quantity, 0)}</strong></span><span><small>Costo final</small><strong>{money((dailySalesQuery.data || []).reduce((sum, item) => sum + item.finalCost, 0))}</strong></span></div>{dailySalesQuery.data?.length ? <div className="daily-sales-table-wrap"><table className="daily-sales-table"><thead><tr><th>Número</th><th>Fecha y hora</th><th>SKU</th><th>Artículo</th><th>Cantidad</th><th>Precio final</th><th>Total</th><th>Origen</th></tr></thead><tbody>{dailySalesQuery.data.map(item => <tr key={item.id}><td><strong>VENTA-{dailySalesDate.replace(/-/g, "")}-{String(item.dailyNumber).padStart(3, "0")}</strong></td><td>{new Date(item.createdAt).toLocaleString("es-DO")}</td><td>{item.sku}</td><td>{item.name}</td><td>{item.quantity}</td><td>{money(item.unitPrice)}</td><td>{money(item.finalCost)}</td><td>{item.source}</td></tr>)}</tbody></table></div> : <div className="orders-empty"><TrendingDown size={34} /><h3>No hay salidas para esta fecha</h3><p>Cuando se descuente un artículo, aparecerá aquí con su precio final.</p></div>}</>}</section> : currentQuery.isLoading ? <div className="orders-state"><LoaderCircle className="orders-spin" />Cargando cotizaciones…</div> : currentQuery.isError ? <div className="orders-state"><h2>No se pudo cargar la vista</h2><p>Verifica tu sesión autorizada e inténtalo nuevamente.</p><button className="orders-button" onClick={() => currentQuery.refetch()}>Reintentar</button></div> : visibleOrders.length === 0 ? <div className="orders-empty"><ClipboardList size={38} /><h2>{isDeletedView ? "No hay cotizaciones archivadas" : "No hay cotizaciones recibidas"}</h2><p>{isDeletedView ? "El historial aparecerá aquí." : "Las nuevas solicitudes aparecerán cuando un cliente envíe su carrito."}</p></div> : <section className="orders-list quotes-list">{visibleOrders.map(({ order, items }) => {
