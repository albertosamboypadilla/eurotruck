...
export default function Inventory() {
  ...
  const [activeScanProduct, setActiveScanProduct] = useState<InventoryCatalogProduct | null>(null);
  ...
  const createArticle = trpc.inventory.create.useMutation({
    onSuccess: created => {
      ...
      setActiveScanProduct(createdProduct);
  ...
  return <main className="inventory-page inventory-page--redesigned inventory-first-stage">
    ...
    <section className="inventory-workspace">
      <div className="inventory-scan-panel">
        ...
        <form className="inventory-scan-form" onSubmit={submitScan}><label htmlFor="inventory-scan">Referencia, SKU o GTIN</label><div><ScanLine size={18} /><input ref={inputRef} id="inventory-scan" autoFocus value={scanCode} onChange={event => setScanCode(event.target.value)} placeholder={catalogLoading ? "Cargando catálogo…" : entryMode === "scan" ? "Escanea el código: suma +1 automáticamente" : "Escribe o escanea para abrir el artículo…"} disabled={catalogLoading || recordCount.isPending} />{entryMode === "manual" && <button className="orders-button" type="submit" disabled={catalogLoading || recordCount.isPending || !scanCode.trim()}><PackageSearch size={15} />Abrir artículo</button>}</div></form>{activeScanProduct && <div className="inventory-active-scan-target" role="status"><div><span className="orders-eyebrow">OBJETIVO DE ESCANEO</span><strong>{activeScanProduct.sku}</strong><small>{activeScanProduct.name} · Las próximas lecturas sumarán automáticamente en {tramo} / {gondola}.</small></div><button type="button" className="inventory-active-scan-clear" onClick={() => { setActiveScanProduct(null); setSelectedProduct(null); setScanCode(""); setLastScanTotal(null); inputRef.current?.focus(); }}>Cambiar referencia</button></div>}
