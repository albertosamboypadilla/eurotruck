...
export default function Inventory() {
  ...
  const [armedScanCode, setArmedScanCode] = useState("");
  ...
  const [clearLocationOpen, setClearLocationOpen] = useState(false);
  const [clearLocationKey, setClearLocationKey] = useState("");
  const [clearLocationError, setClearLocationError] = useState("");
  ...
  const recordSourceRef = useRef<"auto" | "manual">("manual");
  ...
  const recordCount = trpc.inventory.record.useMutation({ onSuccess: result => { playScanTone("success"); if (recordSourceRef.current === "manual") setNotice(buildInventoryNotice(result.wasAlreadyCounted, Number(quantity), result.totalQuantity)); else setNotice(""); setLastScanTotal(result.totalQuantity); setScanCode(""); setQuantity("1"); recordSourceRef.current = "manual"; void Promise.all([inventoryQuery.refetch(), recentIngressQuery.refetch()]); if (expandedInventoryId === result.id) void historyQuery.refetch(); focusScanInput(); }, onError: mutationError => { playScanTone("alert"); setLastScanAutoRecorded(false); setLastScanTotal(null); setError(mutationError.message || "No se pudo guardar el conteo."); recordSourceRef.current = "manual"; focusScanInput(); } });
  ...
  const clearLocation = trpc.inventory.clearLocation.useMutation({ onSuccess: async result => { setClearLocationOpen(false); setClearLocationKey(""); setClearLocationError(""); setNotice(result.found ? `Se borraron ${result.removedUnits} unidades de ${result.tramo} / ${result.gondola}. Los artículos permanecen en el catálogo.` : `No había conteos registrados en ${result.tramo} / ${result.gondola}.`); setSelectedProduct(null); setLastScanAutoRecorded(false); await Promise.all([inventoryQuery.refetch(), recentIngressQuery.refetch()]); focusScanInput(); }, onError: mutationError => setClearLocationError(mutationError.message || "No se pudo borrar el conteo de esta ubicación.") });
  ...
  const createArticle = trpc.inventory.create.useMutation({
    onSuccess: created => {
      ...
      setNewArticleError("");
      setNotice("Artículo nuevo agregado. Registra ahora su cantidad, tramo y estante.");
      setError("");
  ...
  const findProduct = async (code: string, autoRecord = true) => {
    ...
    try {
      ...
      if (!product) { const unknownCode = code.trim(); playScanTone("alert"); const isBarcode = /^\d{6,14}$/.test(unknownCode); setSelectedProduct(null); setArmedScanCode(""); setError(""); setNewArticle({ ...emptyArticle, sku: isBarcode ? "" : unknownCode, barcode: isBarcode ? unknownCode : "" }); setNewArticleImageFile(null); setNewArticleImagePreview(""); setNewArticleError(`Código no encontrado: ${unknownCode}. Completa los datos para agregarlo al catálogo.`); setShowNewArticle(true); return; }
      setSelectedProduct(product);
      setArmedScanCode(code.trim());
      ...
      if (autoRecord) { recordSourceRef.current = "auto"; recordCount.mutate({ productId: product.id, sku: product.sku, name: product.name, description: product.description, brand: product.brand, application: product.application, image: product.image, internalCode: product.internalCode || undefined, barcode: product.barcode || product.gtins?.[0] || undefined, quantity: 1, tramo: location.tramo, gondola: location.gondola }); }
    } catch {
      setSelectedProduct(null); setError("No fue posible cargar el catálogo para escanear artículos.");
    } finally { setCatalogLoading(false); }
  };

  const armScanTarget = (product: InventoryCatalogProduct, code: string) => {
    setSelectedProduct(product);
    setArmedScanCode(code.trim());
    setScanCode("");
    setLastScanAutoRecorded(false);
    setLastScanTotal(null);
    setError("");
    setNotice(`Objetivo seleccionado: ${code.trim()}. Escanea varias veces para sumar automáticamente.`);
  };
  const selectProduct = async (product: InventoryCatalogProduct) => {
    setSelectedProduct(null); setArmedScanCode(""); setLastScanAutoRecorded(false); setLastScanTotal(null); setScanCode(""); setError(""); setNotice(""); setCatalogLoading(true);
    try {
  ...
  useEffect(() => {
    ...
    if (!shouldAutoRegisterInventoryScan(candidate, knownProduct, recordCount.isPending, armedScanCode)) return;
    const timer = window.setTimeout(() => {
      void findProduct(candidate, true);
    }, knownProduct ? 90 : 220);
    return () => window.clearTimeout(timer);
  }, [scanCode, entryMode, recordCount.isPending, allProducts, armedScanCode]);
  ...
  const downloadLowStockExcel = () => {
    ...
    const headers = ["SKU / DT Spare Parts", "Artículo", "Existencia actual", "Mínimo operativo", "Unidades a comprar", "Tramo", "Estante", "Costo", "Precio final", "Código Zebra", "Barcode", "GTIN"];
    const table = buildInventoryExportHtml(headers, rows);
    const blob = new Blob(["\\ufeff", table], { type: "application/vnd.ms-excel;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `EUROTRUCK_Pedido_Bajo_Stock_${new Date().toISOString().slice(0, 10)}.xls`;
  ...
  const downloadInventoryExcel = () => {
    ...
    const headers = ["Código interno Zebra", "Código de barras", "SKU / referencia", "GTIN", "Nombre", "Descripción", "Marca", "Aplicación", "Existencia", "Tramo", "Estante", "Lecturas", "Costo", "Precio final", "Imagen", "Fuente"];
    const table = buildInventoryExportHtml(headers, rows);
    const blob = new Blob(["\\ufeff", table], { type: "application/vnd.ms-excel;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
  ...
  return <main className="inventory-page inventory-page--redesigned inventory-first-stage">
    ...
    <section className="inventory-kpis" aria-label="Resumen de inventario"><button type="button" className={`inventory-kpi-button${countedItems.length === 0 ? " is-disabled" : ""}`} onClick={downloadCountedInventoryExcel} disabled={countedItems.length === 0} aria-label="Descargar reporte de artículos contados"><Boxes size={22} /><span><small>Artículos contados</small><strong>{countedItems.length.toLocaleString("es-DO")}</strong><em>Descargar inventario</em></span><Download size={15} /></button><article><Barcode size={22} /><span><small>Unidades en almacén</small><strong>{totalUnits.toLocaleString("es-DO")}</strong></span></article><button type="button" className={`inventory-kpi-button${lowStockItems ? " is-warning" : ""}`} onClick={downloadLowStockExcel} disabled={!lowStockItems} aria-label="Descargar pedido de compra de artículos bajo stock"><AlertTriangle size={22} /><span><small>Pedido bajo stock</small><strong>{lowStockItems}</strong><em>Descargar pedido</em></span><Download size={15} /></button><article><MapPin size={22} /><span><small>Área activa</small><strong>{tramo} / {gondola}</strong></span></article></section>
    <section className="inventory-workspace">
      <div className="inventory-scan-panel">
        <div className="inventory-scan-location"><div><span className="orders-eyebrow">01 / ÁREA DE CONTEO</span><strong>Ubicación activa</strong><small>Selecciona o agrega el tramo y el estante antes de escanear. Cada lectura se guardará automáticamente en esta área hasta que la cambies.</small><div className="inventory-location-picker"><select value={activeLocationId} onChange={event => selectLocation(event.target.value)} aria-label="Seleccionar ubicación activa">{savedLocations.map(location => <option key={location.id} value={location.id}>{location.tramo} / {location.gondola}</option>)}</select><button type="button" className="inventory-location-delete" disabled={activeLocationId === defaultInventoryLocation.id} onClick={() => { if (window.confirm("¿Eliminar esta área guardada? Las lecturas existentes conservarán su ubicación histórica.")) removeLocation(activeLocationId); }}><Trash2 size={14} />Eliminar área</button><button type="button" className="inventory-location-clear" onClick={() => { setClearLocationKey(""); setClearLocationError(""); setClearLocationOpen(true); }}><Trash2 size={14} />Borrar conteo</button></div><button type="button" className="inventory-location-add" onClick={() => setShowLocationForm(current => !current)}><Plus size={13} />{showLocationForm ? "Cerrar alta de área" : "Agregar otra área"}</button></div><div className="inventory-scan-location-fields"><label>Tramo<input value={tramo} readOnly aria-readonly="true" /></label><label>Estante<input value={gondola} readOnly aria-readonly="true" /></label></div>{showLocationForm && <form className="inventory-location-form" onSubmit={submitNewLocation}><label>Nuevo tramo<input required value={newLocationTramo} onChange={event => setNewLocationTramo(event.target.value)} placeholder="Ej. T-03" /></label><label>Nuevo estante<input required value={newLocationGondola} onChange={event => setNewLocationGondola(event.target.value)} placeholder="Ej. E-12" /></label><button type="submit" className="orders-button orders-button--inventory"><Plus size={14} />Guardar y activar</button></form>}</div>
        ...
        <div className="inventory-mode-switch" role="tablist" aria-label="Modo de entrada"><button type="button" className={entryMode === "scan" ? "is-active" : ""} onClick={() => { setEntryMode("scan"); setArmedScanCode(""); setLastScanAutoRecorded(false); }}><ScanLine size={17} /><span><b>Escaneo automático</b><small>Cada lectura suma +1</small></span></button><button type="button" className={entryMode === "manual" ? "is-active" : ""} onClick={() => { setEntryMode("manual"); setArmedScanCode(""); setLastScanAutoRecorded(false); }}><Keyboard size={17} /><span><b>Entrada manual</b><small>Busca y escribe la cantidad</small></span></button></div>
        <form className="inventory-scan-form" onSubmit={submitScan}><label htmlFor="inventory-scan">Referencia, SKU o GTIN</label><div><ScanLine size={18} /><input ref={inputRef} id="inventory-scan" autoFocus value={scanCode} onChange={event => setScanCode(event.target.value)} placeholder={catalogLoading ? "Cargando catálogo…" : entryMode === "scan" ? "Escanea varias veces: suma automáticamente" : "Escribe o escanea para abrir el artículo…"} disabled={catalogLoading || recordCount.isPending} />{entryMode === "manual" && <button className="orders-button" type="submit" disabled={catalogLoading || recordCount.isPending || !scanCode.trim()}><PackageSearch size={15} />Abrir artículo</button>}</div></form>
        {armedScanCode && selectedProduct && <div className={`inventory-active-scan-target${lastScanAutoRecorded ? " is-success" : ""}`} role="status" aria-label={lastScanAutoRecorded ? `Lectura agregada: ${armedScanCode}` : `Código seleccionado: ${armedScanCode}`}><strong>{armedScanCode}</strong></div>}
        ...
        {clearLocationOpen && <div className="inventory-location-dialog inventory-location-clear-dialog" role="dialog" aria-modal="true" aria-labelledby="clear-location-dialog-title"><form className="inventory-location-dialog-card" onSubmit={event => { event.preventDefault(); setClearLocationError(""); clearLocation.mutate({ tramo, gondola, confirmationKey: clearLocationKey }); }}><div className="inventory-location-dialog-icon"><Trash2 size={30} /></div><span className="orders-eyebrow">BORRADO DE CONTEO</span><h2 id="clear-location-dialog-title">Borrar todo lo contado aquí</h2><p>Se eliminarán únicamente las lecturas de <strong>{tramo} / {gondola}</strong>. Los artículos, SKU, GTIN, precios y catálogo permanecerán intactos.</p><label>Contraseña de confirmación<input type="password" inputMode="numeric" maxLength={4} autoFocus value={clearLocationKey} onChange={event => setClearLocationKey(event.target.value.replace(/\D/g, "").slice(0, 4))} placeholder="1989" autoComplete="off" /></label>{clearLocationError && <div className="inventory-alert inventory-alert--error" role="alert"><AlertTriangle size={16} />{clearLocationError}</div>}<div className="inventory-location-dialog-actions"><button type="button" className="orders-button orders-button--ghost" onClick={() => { setClearLocationOpen(false); setClearLocationKey(""); setClearLocationError(""); }}>Cancelar</button><button type="submit" className="orders-button inventory-location-confirm inventory-location-clear-confirm" disabled={clearLocation.isPending || clearLocationKey.length !== 4}><Trash2 size={17} />{clearLocation.isPending ? "Borrando…" : "Borrar todo"}</button></div></form></div>}
        ...
        <div className="inventory-catalog-list" aria-live="polite">{catalogLoading && catalogProducts.length === 0 ? <div className="inventory-empty"><LoaderCircle className="orders-spin" />Cargando todos los artículos…</div> : visibleCatalogProducts.length === 0 ? <div className="inventory-empty"><PackageSearch size={28} /><p>No hay artículos que coincidan con ese filtro.</p></div> : visibleCatalogProducts.map(product => { const counted = countedByProduct.get(product.id); const lowStock = Boolean(counted && counted.totalQuantity <= lowStockThreshold); return <div className="inventory-catalog-row-wrap" key={product.id}><article className={`inventory-catalog-row${counted ? " is-counted" : ""}${lowStock ? " is-low-stock" : ""}`} role="button" tabIndex={0} onClick={() => selectProduct(product)} onKeyDown={event => { if ((event.key === "Enter" || event.key === " ") && event.target === event.currentTarget) { event.preventDefault(); selectProduct(product); } }}><span className="inventory-catalog-thumb">{product.image ? <img src={product.image} alt="" loading="lazy" /> : <Boxes size={18} />}</span><span className="inventory-catalog-copy"><button type="button" className="inventory-code-target" onClick={event => { event.stopPropagation(); armScanTarget(product, product.sku); }} aria-label={`Seleccionar referencia ${product.sku}`}><strong>{product.sku}</strong></button>{product.internalCode && <em className="inventory-internal-code">Zebra {product.internalCode}</em>}{product.barcode && <em className="inventory-barcode">Barras {product.barcode}</em>}{product.gtins?.length ? <span className="inventory-gtin-targets">{product.gtins.map(gtin => <button type="button" className="inventory-code-target inventory-code-target--gtin" key={gtin} onClick={event => { event.stopPropagation(); armScanTarget(product, gtin); }} aria-label={`Seleccionar GTIN ${gtin}`}>GTIN {gtin}</button>)}</span> : <em className="inventory-gtin-missing">GTIN no registrado</em>}<small className="inventory-catalog-location"><MapPin size={12} />{counted ? `Última ubicación: ${counted.lastTramo || "—"} / ${counted.lastGondola || "—"}` : "Ubicación pendiente"}</small><b>{product.name}</b><small>{product.brand || "—"} · {product.application || "Aplicación general"}</small></span><span className={`inventory-catalog-status${lowStock ? " is-low-stock" : ""}`}>{counted ? lowStock ? <><AlertTriangle size={15} />Bajo stock · {counted.totalQuantity}</> : <><CheckCircle2 size={15} />Contado · {counted.totalQuantity}</> : "Pendiente"}</span></article><button type="button" className="inventory-catalog-sale-button" onClick={() => { setSaleCode(product.sku); setSaleQuantity("1"); setSaleConfirmSku(product.sku); setSaleConfirmKey(""); setSaleConfirmError(""); }} aria-label={`Eliminar ${product.sku} del inventario`}>Eliminar</button></div>; })}</div>
        ...
        {selectedProduct && <form className="inventory-product-card inventory-product-card--detail" onSubmit={submitCount}>
          ...
          {!lastScanAutoRecorded && <><div className="inventory-count-fields"><label>Cantidad manual / caja<input type="number" min="1" max="9999" step="1" inputMode="numeric" required value={quantity} onChange={event => setQuantity(event.target.value)} placeholder="Ej. 12" aria-label="Cantidad manual o unidades por caja" /><small className="inventory-field-hint">Indica cuántas unidades contiene la caja o cuántas piezas vas a contar.</small></label><label>Tramo<input required value={tramo} readOnly aria-readonly="true" /></label><label>Estante<input required value={gondola} readOnly aria-readonly="true" /></label></div><button className="orders-button inventory-save-button" disabled={recordCount.isPending} type="submit"><Plus size={16} />{recordCount.isPending ? "Guardando…" : "Sumar cantidad manual"}</button></>}
        </form>}
