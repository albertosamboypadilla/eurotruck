...
import { AlertTriangle, Barcode, Boxes, CheckCircle2, ChevronLeft, ChevronRight, DollarSign, Eye, House, Keyboard, LoaderCircle, LogIn, LogOut, MapPin, PackageSearch, Plus, Printer, RefreshCw, ScanLine, Trash2, TrendingDown, X } from "lucide-react";
...
const catalogIndexUrl = "/manus-storage/index_3c4a197f.json";
...
const catalogShardUrls: Record<string, string> = {
  "0": "/manus-storage/shard-0_f5ee8588.json",
  "1": "/manus-storage/shard-1_a9a563b7.json",
  "2": "/manus-storage/shard-2_3e516df1.json",
  "3": "/manus-storage/shard-3_89d14041.json",
  "4": "/manus-storage/shard-4_f3d062ed.json",
  "5": "/manus-storage/shard-5_e83cb19f.json",
  "6": "/manus-storage/shard-6_99838807.json",
  "7": "/manus-storage/shard-7_073087bd.json",
  "8": "/manus-storage/shard-8_37c5f18e.json",
  "9": "/manus-storage/shard-9_ae522da4.json",
  s: "/manus-storage/shard-s_f88792cd.json",
};
const emptyArticle = { sku: "", name: "", description: "", brand: "", application: "", image: "", costPrice: "0", salePrice: "0" };
...
export default function Inventory() {
  ...
  }, [selectedProduct?.id, selectedInventory?.costPrice, selectedInventory?.salePrice]);
  ...
  const findProduct = async (code: string, autoRecord = true) => {
    ...
    try {
      ...
      setLocationWarning(isDifferentLocation ? `Atención: ${product.sku} ya tiene ${existing?.totalQuantity} unidad${existing?.totalQuantity === 1 ? "" : "es"} en ${existing?.lastTramo || "—"} / ${existing?.lastGondola || "—"}. Esta nueva entrada quedará en ${location.tramo} / ${location.gondola}.` : "");
      setLastScanAutoRecorded(autoRecord);
      setLastScanTotal(null);
      if (autoRecord) recordCount.mutate({ productId: product.id, sku: product.sku, name: product.name, description: product.description, brand: product.brand, application: product.application, image: product.image, quantity: 1, tramo: location.tramo, gondola: location.gondola });
    } catch {
  ...
  const submitNewArticle = (event: FormEvent) => { event.preventDefault(); setNewArticleError(""); createArticle.mutate({ sku: newArticle.sku.trim(), name: newArticle.name.trim(), description: newArticle.description.trim() || undefined, brand: newArticle.brand.trim() || undefined, application: newArticle.application.trim() || undefined, image: newArticle.image.trim() || undefined, costPrice: Number(newArticle.costPrice || 0), salePrice: Number(newArticle.salePrice || 0) }); };
  const savePricing = () => {
  ...
  return <main className="inventory-page inventory-page--redesigned">
    ...
    <section className="inventory-workspace">
      <div className="inventory-scan-panel">
        ...
        {locationWarning && <div className="inventory-alert inventory-alert--warning inventory-location-warning" role="alert"><AlertTriangle size={18} /><span><strong>Artículo en otra ubicación</strong>{locationWarning}</span></div>}
        ...
        {selectedProduct && <form className="inventory-product-card inventory-product-card--detail" onSubmit={submitCount}>
          ...
          <div className="inventory-label-panel"><div><strong>Etiqueta Zebra LP2824 Plus</strong><small>Genera etiquetas ZPL con código interno, SKU y nombre.</small></div><div className="inventory-label-fields"><label>Inicio<input value={labelStartCode} onChange={event => setLabelStartCode(event.target.value.replace(/\D/g, "").slice(-12))} inputMode="numeric" minLength={5} /></label><label>Cantidad<input type="number" min="1" max="500" value={labelQuantity} onChange={event => setLabelQuantity(event.target.value)} /></label><button type="button" className="orders-button orders-button--ghost" onClick={downloadZebraLabels}><Printer size={15} />Descargar ZPL</button></div></div>
