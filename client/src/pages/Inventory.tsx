...
export default function Inventory() {
  ...
  const selectProduct = async (product: InventoryCatalogProduct) => {
    setSelectedProduct(null); setLastScanAutoRecorded(false); setLastScanTotal(null); setScanCode(""); setError(""); setNotice(""); setCatalogLoading(true);
    try {
      const fullProduct = product.image ? product : findInventoryCatalogProduct(await loadCatalogForSearch(), product.sku) || product;
      armScanTarget(fullProduct, fullProduct.sku);
    } catch { armScanTarget(product, product.sku); setError("No fue posible cargar el detalle completo; puedes continuar con el conteo."); }
    finally { setCatalogLoading(false); }
  };
  useEffect(() => {
