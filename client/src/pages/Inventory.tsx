  ...
  return <main className="inventory-page inventory-page--redesigned inventory-first-stage">
    ...
    <section className="inventory-workspace">
      <div className="inventory-scan-panel">
        ...
        <section className="inventory-recent-ingress" aria-labelledby="recent-ingress-title"><div className="inventory-subpanel-heading"><div><span className="orders-eyebrow">03 / ÚLTIMOS INGRESOS</span><h3 id="recent-ingress-title">Artículos ingresados recientemente</h3><p>Las lecturas más recientes aparecen aquí con su fecha, cantidad y ubicación.</p></div><RefreshCw size={20} /></div>{recentIngressQuery.isLoading ? <div className="inventory-empty">Cargando ingresos…</div> : recentIngressQuery.data?.length ? <div className="inventory-recent-table-wrap"><table className="inventory-recent-table"><thead><tr><th>Fecha y hora</th><th>SKU</th><th>Artículo</th><th>Código Zebra</th><th>Cantidad</th><th>Ubicación</th><th>Usuario</th></tr></thead><tbody>{(recentIngressQuery.data as RecentIngressRow[]).map(row => <tr key={row.id}><td>{formatInventoryScanDate(row.createdAt)}</td><td><strong>{row.sku}</strong></td><td>{row.name}</td><td>{row.internalCode || "—"}</td><td className="inventory-recent-quantity">+{row.quantity}</td><td>{row.tramo} / {row.gondola}</td><td>{row.countedBy}</td></tr>)}</tbody></table></div> : <div className="inventory-empty"><PackageSearch size={24} /><p>Aún no hay ingresos registrados.</p></div>}</section>
