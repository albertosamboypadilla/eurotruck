...
export default function Inventory() {
  ...
  const findProduct = async (code: string, autoRecord = true) => {
    ...
    try {
      ...
      if (!product) { const unknownCode = code.trim(); const isBarcode = /^\d{6,14}$/.test(unknownCode); setSelectedProduct(null); setError(""); setNewArticle({ ...emptyArticle, sku: isBarcode ? "" : unknownCode, barcode: isBarcode ? unknownCode : "" }); setNewArticleError(`Código no encontrado: ${unknownCode}. Completa los datos para agregarlo al catálogo.`); setShowNewArticle(true); return; }
