...
export default function Inventory() {
  ...
  const salesHistoryQuery = trpc.inventory.salesHistory.useQuery(topSoldInput, { enabled: isAdmin1, retry: false });
  ...
  const confirmProtectedSale = () => {
    ...
    recordSale.mutate({ sku: saleConfirmSku, quantity: requestedQuantity, source: "manual", confirmationKey: saleConfirmKey });
  ...
  const submitNewArticle = (event: FormEvent) => { event.preventDefault(); setNewArticleError(""); const location = getInventoryScanLocation(tramo, gondola); createArticle.mutate({ sku: newArticle.sku.trim(), barcode: newArticle.barcode.trim() || undefined, name: newArticle.name.trim(), description: newArticle.description.trim() || undefined, brand: newArticle.brand.trim() || undefined, application: newArticle.application.trim() || undefined, image: newArticle.image.trim() || undefined, costPrice: Number(newArticle.costPrice || 0), salePrice: Number(newArticle.salePrice || 0), initialQuantity: Number(newArticle.initialQuantity || 0), tramo: location.tramo, gondola: location.gondola }); };
  const savePricing = () => {
  ...
  const submitSale = (event: FormEvent) => { event.preventDefault(); setSaleError(""); setSaleNotice(""); const parsedQuantity = Number(saleQuantity); if (!saleCode.trim() || !Number.isInteger(parsedQuantity) || parsedQuantity < 1) { setSaleError("Indica un SKU, código Zebra o barcode y una cantidad válida."); return; } const matched = findInventoryCatalogProduct(allProducts, saleCode.trim()); setSaleConfirmSku(matched?.sku || saleCode.trim()); setSaleQuantity(String(parsedQuantity)); setSaleConfirmKey(""); setSaleConfirmError(""); };
  const printTopSoldReport = () => { window.print(); };
  const downloadSalesHistoryExcel = () => {
    const history = (salesHistoryQuery.data as SaleHistoryRow[] | undefined) || [];
    const rows = history.map(item => [item.id, item.sku, item.name, item.quantity, formatInventoryScanDate(item.createdAt), new Date(item.createdAt).toISOString(), item.movedBy, item.source, item.orderId ?? ""]);
    const headers = ["ID movimiento", "SKU / DT Spare Parts", "Artículo", "Cantidad descontada", "Fecha y hora local", "Fecha y hora UTC", "Usuario", "Origen", "Orden"];
    const table = buildInventoryExportHtml(headers, rows);
    const blob = new Blob(["\\ufeff", table], { type: "application/vnd.ms-excel;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `historial-mas-vendidos-${reportYear}-${String(reportMonth).padStart(2, "0")}.xls`;
  ...
  const downloadInventoryExcel = () => {
    const rows = allProducts.map(product => {
      ...
      const gtins = product.gtins?.join(" · ") || getGtinsForSku(product.sku, gtinMap).join(" · ");
  ...
  const lowStockItems = countedItems.filter(item => item.totalQuantity <= lowStockThreshold).length;

  return <main className="inventory-page inventory-page--redesigned">
    ...
    <section className="inventory-kpis" aria-label="Resumen de inventario"><article><Boxes size={22} /><span><small>Artículos contados</small><strong>{countedItems.length.toLocaleString("es-DO")}</strong></span></article><article><Barcode size={22} /><span><small>Unidades en almacén</small><strong>{totalUnits.toLocaleString("es-DO")}</strong></span></article><article className={lowStockItems ? "is-warning" : ""}><AlertTriangle size={22} /><span><small>Bajo stock</small><strong>{lowStockItems}</strong></span></article><article><MapPin size={22} /><span><small>Área activa</small><strong>{tramo} / {gondola}</strong></span></article></section>
    <section className="inventory-workspace">
      <div className="inventory-scan-panel">
        ...
        <div className="inventory-panel-heading"><div><span className="orders-eyebrow">02 / ENTRADA</span><h2>Registrar artículos</h2></div><Barcode size={30} /></div>
        ...
        <form className="inventory-scan-form" onSubmit={submitScan}><label htmlFor="inventory-scan">Referencia, SKU o GTIN</label><div><ScanLine size={18} /><input ref={inputRef} id="inventory-scan" autoFocus value={scanCode} onChange={event => setScanCode(event.target.value)} placeholder={catalogLoading ? "Cargando catálogo…" : entryMode === "scan" ? "Escanea y suma automáticamente…" : "Escribe o escanea para abrir el artículo…"} disabled={catalogLoading || recordCount.isPending} /><button className="orders-button" type="submit" disabled={catalogLoading || recordCount.isPending || !scanCode.trim()}><PackageSearch size={15} />{recordCount.isPending ? "Guardando…" : entryMode === "scan" ? "Registrar +1" : "Abrir artículo"}</button></div></form>
